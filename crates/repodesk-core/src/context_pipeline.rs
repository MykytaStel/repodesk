...
impl ContextPipelineSnapshot {
    pub fn new(
        ...
        let candidate_tokens = candidates
            .iter()
            .fold(0usize, |total, candidate| total.saturating_add(candidate.candidate_tokens));
        let included_tokens = selections
            .iter()
            .fold(0usize, |total, selection| total.saturating_add(selection.included_tokens));
    ...
    pub fn validate(&self) -> Result<(), ContextPipelineValidationError> {
        ...
        for candidate in &self.candidates {
            ...
            validate_score(candidate.id.as_str(), "relevance", candidate.relevance_score)?;
            validate_score(candidate.id.as_str(), "freshness", candidate.freshness_score)?;
