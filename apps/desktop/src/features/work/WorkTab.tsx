...
import {
  ExecutionPacketPreview,
  type ExecutionPacketPreviewData,
} from "./ExecutionPacketPreview";
import { ReviewPanel } from "./ReviewPanel";
import {
  approvalSemanticState,
  phaseSemanticState,
} from "./workSemantic";
import type { Phase, PhaseProgress } from "../../shared/api/orchestrate";
...
export function WorkTab({ setActiveTab }: { setActiveTab: (tab: TabId) => void }) {
  ...
  if (phase.isLoading || !phase.data) {
    return <LoadingState message="Loading Work Item flow…" scope="surface" />;
  }

  const progress = phase.data;
  const current = progress.phases.find((item) => item.phase === progress.current) ?? progress.phases[0];
  const currentSemantic = phaseSemanticState(current.status, true);
  ...
  const preview = (strategyPreview?.execution as ExecutionPacketPreviewData | undefined) ?? null;
  ...
  const approvalSemantic = approvalSemanticState(executeApprovalsMet);
      ...
      <section className="work-focus-card">
        <PanelHeader
          ...
          trailing={latest && !latest.dry_run ? (
            <div className="work-run-facts" aria-label="Latest run facts">
              ...
              <Metric
                label="Tokens"
                value={(latest.total_input_tokens + latest.total_output_tokens).toLocaleString()}
                tone="info"
              />
              <Metric label="Cost" value={latest.total_cost_units.toFixed(3)} tone="info" />
            </div>
          ) : undefined}
        />
        ...
        <ol className="phase-rail compact" aria-label="Task phases">
          {progress.phases.map((view) => {
            const isCurrent = view.phase === progress.current;
            const semantic = phaseSemanticState(view.status, isCurrent);
            return (
              <li
                key={view.phase}
                className={`phase-chip phase-${view.status}${isCurrent ? " phase-current" : ""}`}
              ...
              >
                <StatusBadge
                  ...
                  tone={semantic.tone}
                  role="status"
                  ariaLabel={`Phase ${view.title}: ${semantic.label}`}
                />
              </li>
            );
          })}
        ...
        <EvidenceState
          label="Phase intent"
          state={progress.complete ? "Complete" : currentSemantic.label}
          tone={progress.complete ? "positive" : currentSemantic.tone}
          detail={copy.detail}
        >
          ...
          <div className="phase-controls compact">
            {!hasProject ? (
              ...
              <EvidenceState
                ...
                state="Ready"
                tone="positive"
                detail="Project and Work Item are ready. Continue to Prepare."
              />
            )}
          </div>
        ) : null}
              ...
              <>
                ...
                <div className="exec-approval-gates">
                  <PanelHeader
                    ...
                    trailing={(
                      <StatusBadge
                        label={approvalSemantic.label}
                        tone={approvalSemantic.tone}
                        role="status"
                        ariaLabel={`Launch approvals: ${approvalSemantic.label}`}
                      />
                    )}
                  />
        ...
        {sharedPrimaryOwner ? (
          <ActionBar
            ...
            detail={executeBlocked
              ? "Refresh the strategy packet and grant its required approvals before launch."
              : undefined}
          />
        ) : null}

        {mutationError ? (
          <ErrorState title="Work action failed" detail={mutationError} scope="inline" />
        ) : null}
      </section>
    </div>
  );
